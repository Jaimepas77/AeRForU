...
export async function downloadProblems() {
  try {
    ...
    for (const submission of submissionsIDs) {
        let submissionCode = await getSubmissionCode(submission.num, cookieValue);
        //Add to zip
        ...
        let filename = submission.problemId + " - " + submission.problemName + "." + extension;
...
async function getSubmissionIDs(userID) {
    ...
    problem_submissions_base_url = problem_submissions_base_url.replace("${userID}", userID);
    
    //Loop through all problems [100...] until no more problems
    let problemId = 100;
    let submissionsIDs = [];
    while (true) { //Infinite loop, break when no more problems
        //Get problem name
        let problem_info_base_url = "https://aceptaelreto.com/ws/problem/${problemId}";
        let problem_info_url = problem_info_base_url.replace("${problemId}", problemId);
        let request_info = await fetch(problem_info_url);
        if (request_info.status === 404) {
            //No more problems
            console.log("No more problems found");
            break;
        }
        let problem_info = await request_info.json();
        let problemName = problem_info.title;
        //console.log("Problem " + problemId + ": " + problemName);

        let problem_submissions_url = problem_submissions_base_url.replace("${problemId}", problemId);

        let request = await fetch(problem_submissions_url);
        if (request.status === 404) {
            //No more problems
            console.log("No more problems found");
            console.log("URL: " + problem_submissions_url);
            break;
        }
        ...
        while (submissions.submission.length > 0 && !end) {
            ...
            for (const submission of submissions.submission) {
                if (submission.result === "AC") {
                    ...
                    submissionsIDs.push({ num: submission.num, problemId: problemId, problemName: problemName, language: submission.language });
                    end = true;
